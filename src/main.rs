...
fn main() {
    ...
    while !rl.window_should_close() {
        ...
        let color = ((frame_number / 5 % 100) as u8, (frame_number / 5 % 155) as u8, 255);

        if frame_number % 5 == 0 && particles.len() <= 1500 {

            for i in 0..6 {
                let pos = (25 + i * 20) as f32;
                particles.push(VerletObject::new(Vec2::new(pos, 16.0), Vec2::new(pos - 1.0, 15.0), Vec2::new(0.0, 0.0), PARTICLE_SIZE, color));
            }
        }
        
        rl.set_target_fps(60);
        solver.update(&mut particles, 1.0 / rl.get_fps() as f32, (PARTICLE_SIZE.powf(1.5)+1.0) as u32);
        ...
        for p in particles.iter() {
            let col = p.col;
            d.draw_circle(p.position_current.x as i32, p.position_current.y as i32, p.radius, Color::new(col.0, col.1, col.2, 255));
        }
        d.draw_fps(0, 0);
    }
}
